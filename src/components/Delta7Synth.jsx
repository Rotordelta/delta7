...
export default function Delta7Synth() {
  ...
  const clearLooperBuffer = () => {
    ...
    showEditorStatus("Loop Cleared 🗑️");
  };
  ...
  const undoLastLooperAction = () => {
    ...
    if (!slot) return;
    if (!slot.undoBuffer) {
      showEditorStatus("Nothing to undo ✖️");
      return;
    }
    ...
    setSampleSlots(nextSlots);
    setLooperHasUndo(false);
    showEditorStatus("Undone last overdub ↩️");
...
grainSource.buffer = isRevB && currentRevBuf ? currentRevBuf : currentBuf;
  ...
  // Shared stopPropagation handler — replaces hundreds of (e) => e.stopPropagation() closures
  ...
  const handleLoopBadgeClick = useCallback((e) => {
    e.stopPropagation();
    const pad = e.currentTarget.closest('.perf-pad[data-deck]');
    if (!pad) return;
    const deck = pad.dataset.deck;
  ...
  const handleTriggerModeBadgeClick = useCallback((e) => {
    e.stopPropagation();
    const pad = e.currentTarget.closest('.perf-pad[data-deck]');
    if (!pad) return;
    const deck = pad.dataset.deck;
        ...
        <div className="screen-back" style={{ padding: '0.5rem' }}>
          <div className="lcd-bezel-shadow" style={{ height: '100%', border: '1px solid #ffe600', background: 'radial-gradient(circle at center, #0b0f19 0%, #010408 100%)', display: 'flex', flexDirection: 'column' }}>
            ...
            <PerformanceView
              ...
              focusZoomDuration={focusZoomDuration}
              isFocusZoomActive={isFocusZoomActive}
              perfEvents={perfEvents}
              setPerfEvents={setPerfEvents}
              highwayZoom={highwayZoom}
              setHighwayZoom={setHighwayZoom}
              perfTimeSignature={perfTimeSignature}
              perfSeqLength={perfSeqLength}
              highwayEditMode={highwayEditMode}
              resizeDragTarget={resizeDragTarget}
              setResizeDragTarget={setResizeDragTarget}
              selectedPill={selectedPill}
              setSelectedPill={setSelectedPill}
              sampleSlots={sampleSlots}
              setSampleSlots={setSampleSlots}
              updateSlotParam={updateSlotParam}
              saveSlotMetadataToDb={saveSlotMetadataToDb}
              platterAngleARef={platterAngleARef}
              platterAngleBRef={platterAngleBRef}
              platterRefA={platterRefA}
              platterRefB={platterRefB}
              ringsContainerRefA={ringsContainerRefA}
              ringsContainerRefB={ringsContainerRefB}
              handlePlatterMouseDown={handlePlatterMouseDown}
              handlePlatterMouseMove={handlePlatterMouseMove}
              handlePlatterMouseUp={handlePlatterMouseUp}
              handlePlatterClick={handlePlatterClick}
              handleRingsMouseMove={handleRingsMouseMove}
              handleRingsMouseLeave={handleRingsMouseLeave}
              targetCirclesRefsA={targetCirclesRefsA}
              targetCirclesRefsB={targetCirclesRefsB}
              laneBgsRefsA={laneBgsRefsA}
              laneBgsRefsB={laneBgsRefsB}
              highwayEventsRefA={highwayEventsRefA}
              highwayEventsRefB={highwayEventsRefB}
              handleHighwayMouseDown={handleHighwayMouseDown}
              handleHighwayMouseMove={handleHighwayMouseMove}
              handleHighwayMouseUp={handleHighwayMouseUp}
              perfPlaybackActive={perfPlaybackActive}
              perfRecordActive={perfRecordActive}
              seqTimerDisplayRef={seqTimerDisplayRef}
              seqTimeDisplayRef={seqTimeDisplayRef}
              playVoice={playVoice}
              stopVoice={stopVoice}
              cycleTriggerMode={cycleTriggerMode}
              toggleSlotLoop={toggleSlotLoop}
              getSlotLabel={getSlotLabel}
              stopProp={stopProp}
              deckAPitch={deckAPitch}
              deckBPitch={deckBPitch}
              deckAKeyLock={deckAKeyLock}
              deckBKeyLock={deckBKeyLock}
              deckAStretch={deckAStretch}
              deckBStretch={deckBStretch}
              handleMidiCC={handleMidiCC}
              modWheelVal={modWheelVal}
              userRotorPresets={userRotorPresets}
              handleLoadRotorPreset={handleLoadRotorPreset}
              handleSaveRotorPreset={handleSaveRotorPreset}
              selectedRotorPresetIdx={selectedRotorPresetIdx}
              setSelectedRotorPresetIdx={setSelectedRotorPresetIdx}
              userEchoPresets={userEchoPresets}
              handleLoadEchoPreset={handleLoadEchoPreset}
              handleSaveEchoPreset={handleSaveEchoPreset}
              selectedEchoPresetIdx={selectedEchoPresetIdx}
              setSelectedEchoPresetIdx={setSelectedEchoPresetIdx}
              masterGlueCompressorRef={masterGlueCompressorRef}
              masterLimiterRef={masterLimiterRef}
              masterLimiterActive={masterLimiterActive}
              setMasterLimiterActive={setMasterLimiterActive}
              masterLimiterVuRefsArr={masterLimiterVuRefsArr}
              showExportModal={showExportModal}
              setShowExportModal={setShowExportModal}
              setPerformanceViewActive={setPerformanceViewActive}

              deleteSelectedPill={deleteSelectedPill}
              handleCopyDeck={handleCopyDeck}
              handlePasteDeck={handlePasteDeck}
              highwayClipboard={highwayClipboard}
              handleClearDeck={handleClearDeck}
              seqCurrentBeatRef={seqCurrentBeatRef}
              seqStartBeatOffsetRef={seqStartBeatOffsetRef}
              perfPlaybackActiveRef={perfPlaybackActiveRef}
              audioCtxRef={audioCtxRef}
              perfPlayStartTimeRef={perfPlayStartTimeRef}
              schedulerNodeRef={schedulerNodeRef}
              sortedPerfEventsRef={sortedPerfEventsRef}
              showEditorStatus={showEditorStatus}
              stopPerformancePlayback={stopPerformancePlayback}
              togglePerformancePlayback={togglePerformancePlayback}
              perfIsDubbing={perfIsDubbing}
              perfRecordArmed={perfRecordArmed}
              togglePerformanceRecord={togglePerformanceRecord}
              perfQuantizeMode={perfQuantizeMode}
              setPerfQuantizeMode={setPerfQuantizeMode}
              setPerfSeqLength={setPerfSeqLength}
              clearPerformance={clearPerformance}
              macros={macros}
              macroAssignMode={macroAssignMode}
              setMacroAssignMode={setMacroAssignMode}
              triggerPerfPadInternal={triggerPerfPadInternal}
              inputGainSat={inputGainSat}
              setInputGainSat={setInputGainSat}
              duckerAmount={duckerAmount}
              setDuckerAmount={setDuckerAmount}
              glueMix={glueMix}
              setGlueMix={setGlueMix}
              perfSeqLengthRef={perfSeqLengthRef}
              midiLearnParam={midiLearnParam}
              midiMappings={midiMappings}
              setMidiLearnParam={setMidiLearnParam}
              setPerfTimeSignature={setPerfTimeSignature}
              metronomeOn={metronomeOn}
              stopMetronome={stopMetronome}
              setMetronomeOn={setMetronomeOn}
              startMetronome={startMetronome}
              masterSyncActive={masterSyncActive}
              setMasterSyncActive={setMasterSyncActive}
              ...
              armLooperInput={armLooperInput}
              isArmed={isArmed}
              isLiveRecording={isLiveRecording}
              toggleLiveLoopRecording={toggleLiveLoopRecording}
              liveRecPendingStart={liveRecPendingStart}
              slotMap={slotMap}
              triggerStutterDeck={triggerStutterDeck}
              releaseStutterDeck={releaseStutterDeck}
              setDeckAKeyLock={setDeckAKeyLock}
              setDeckBKeyLock={setDeckBKeyLock}
              setDeckAPitch={setDeckAPitch}
              setDeckBPitch={setDeckBPitch}
              setDeckAStretch={setDeckAStretch}
              setDeckBStretch={setDeckBStretch}
              dragOverDeck={dragOverDeck}
              setDragOverDeck={setDragOverDeck}
              handlePadGridMouseDown={handlePadGridMouseDown}
              handlePadGridMouseUp={handlePadGridMouseUp}
              handlePadGridMouseLeave={handlePadGridMouseLeave}
              handlePadGridTouchStart={handlePadGridTouchStart}
              handlePadGridTouchEnd={handlePadGridTouchEnd}
              handlePadGridContextMenu={handlePadGridContextMenu}
              loadCrateToDeck={loadCrateToDeck}
              selectedEditSlotId={selectedEditSlotId}
              setActiveEqSlotId={setActiveEqSlotId}
              padDomRefsB={padDomRefsB}
              padDomRefsA={padDomRefsA}
              handleRevBadgeClick={handleRevBadgeClick}
              handleLoopBadgeClick={handleLoopBadgeClick}
              handleTriggerModeBadgeClick={handleTriggerModeBadgeClick}
              deckBSoloActive={deckBSoloActive}
              setDeckBSoloActive={setDeckBSoloActive}
              deckASoloActive={deckASoloActive}
              setDeckASoloActive={setDeckASoloActive}
              stopPerfVoice={stopPerfVoice}
              setDeckBPlaying={setDeckBPlaying}
              setDeckAPlaying={setDeckAPlaying}
              deckBPlayBtnRef={deckBPlayBtnRef}
              deckAPlayBtnRef={deckAPlayBtnRef}
              sampleSlotsRef={sampleSlotsRef}
              paramsRef={paramsRef}
              deckBPlayingRef={deckBPlayingRef}
              deckAPlayingRef={deckAPlayingRef}
              setHighwayEditMode={setHighwayEditMode}
              copySelectedPill={copySelectedPill}
              copiedPill={copiedPill}
              pasteSelectedPill={pasteSelectedPill}
              deckAEqHigh={deckAEqHigh}
              setDeckAEqHigh={setDeckAEqHigh}
              deckAEqMid={deckAEqMid}
              setDeckAEqMid={setDeckAEqMid}
              deckAEqLow={deckAEqLow}
              setDeckAEqLow={setDeckAEqLow}
              deckBEqHigh={deckBEqHigh}
              setDeckBEqHigh={setDeckBEqHigh}
              deckBEqMid={deckBEqMid}
              setDeckBEqMid={setDeckBEqMid}
              deckBEqLow={deckBEqLow}
              setDeckBEqLow={setDeckBEqLow}
              vuSegLRefsArr={vuSegLRefsArr}
              vuSegRRefsArr={vuSegRRefsArr}
              deckAVolFader={deckAVolFader}
              setDeckAVolFader={setDeckAVolFader}
              deckBVolFader={deckBVolFader}
              setDeckBVolFader={setDeckBVolFader}
              crossfaderVal={crossfaderVal}
              setCrossfaderVal={setCrossfaderVal}
