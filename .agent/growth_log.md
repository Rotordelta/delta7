...
- **Jimmy's Preferences**:
  ...
  - **Playback Nudge & Lookahead (PDC)**: In addition to recording-stage cropping, introducing a per-slot `PLAYBACK NUDGE (MICRO-TIMING)` slider allows the user to shift playback timing dynamically (e.g., -100ms to +100ms) in real-time. Negative values schedule the buffer start time early (lookahead) to compensate for any remaining hardware/driver dragging, matching the delay-compensation features of professional DAWs.
